...
public class DashboardController {
	...
	// ObjectMapper 란?
	/*
	 * JSON 컨텐츠를 Java 객체로 deserialization 하거나 Java 객체를 JSON으로 serialization 할 때 사용하는
	 * Jackson 라이브러리의 클래스이다.
	 * 
	 * 직렬화 (serialization) : 메모리를 디스크에 저장하거나 네트워크 통신에 사용하기 위한 형식으로 변환하는 것 역직렬화
	 * (desrialization): 그 반대로 디스크에 저장한 데이터를 읽거나, 네트워크 통신으로 받은 데이터를 메모리에 쓸 수 있도록 다시
	 * 변환하는 것
	 */
	...
	public String dashboard(Locale locale, Model model) {
		...
		// 총 작업의 개수 가져오기
		HttpEntity<String> res_TotalJobsCount = restTemplate.exchange(ServerIp_job + "/totalnum", HttpMethod.GET,
				request, String.class);
		System.out.println("response (/totalnum) = " + res_TotalJobsCount);
		// 오늘 등록된 작업의 개수 가져오기
		HttpEntity<String> res_TodayNewJobs = restTemplate.exchange(ServerIp_job + "/addtoday", HttpMethod.GET, request,
				String.class);
		System.out.println("response (/addtoday) = " + res_TodayNewJobs);
		// 스케줄이 등록이 안된 작업의 개수 가져오기
		HttpEntity<String> res_NoScheduleJobs = restTemplate.exchange(ServerIp_job + "/notregist", HttpMethod.GET,
				request, String.class);
		System.out.println("response (/notregist) = " + res_NoScheduleJobs);
		// 오늘 시작된 작업의 개수 가져오기
		HttpEntity<String> res_TodayStartedJobs = restTemplate.exchange(ServerIp_job + "/startschedule", HttpMethod.GET,
				request, String.class);
		System.out.println("response (/startschedule) = " + res_TodayStartedJobs);

		ResponseEntity<List<HostDTO>> getHostResult = restTemplate.exchange(ServerIp_host + "/list", HttpMethod.GET,
		...
		try {
			// 각각의 COUNT(*) 값 뽑기
			// objectMapper.readValue를 사용해서 API에서 응답받은 JSON 형식의 Body를 Map<String,
			// Integer>타입으로 역직렬화
			...
			Map<String, Integer> map_TodayStartedJobs = objectMapper.readValue(res_TodayStartedJobs.getBody(),
					Map.class);
	...
	public List<JobOfHostDTO> GetJobOfHost(Locale locale, Model model, @RequestParam(value = "hostId") String hostId) {
		...
		ResponseEntity<List<JobOfHostDTO>> getHostResult = restTemplate.exchange(
				ServerIp_host + "/" + hostId + "/getjobinfo", HttpMethod.GET, null,
	...
	public List<ChartDataDTO> GetChartGraph(Model model, @RequestParam(value="st") String startDay, 
		...
		// API 호출 코드
		ResponseEntity<List<ChartDTO>> getChartGraph = restTemplate.exchange(
				ServerIp_run + "/getgraph" + "/" + timeStamp , HttpMethod.GET, null,
		...
		System.out.println("X축 결과(dash): " + StartDTList); // 뽑아낸 x축 (StartDT)
		// 중복값 추출
		...
		System.out.println("중복값 결과(dash) : " + duplicateList);
		...
		System.out.println("정렬 결과(dash) : " + duplicateList_sort);
		...
		for (int i = 0; i < duplicateList_sort.size(); i++) {
			...
			for (int j = 0; j < ChartDatalist.size(); j++) {
				...
				if (duplicateList_sort.get(i) == ChartDatalist.get(j).JobId) {
					...
					if(ChartDatalist.get(j).Duration == null) {
						startDT_Duration.add((long) 0);
					}
					startDT_Duration.add((long)(Double.parseDouble(ChartDatalist.get(j).Duration)*1000));
					System.out.print(millis + ", ");
					System.out.print((long)(Double.parseDouble(ChartDatalist.get(j).Duration)*1000) + ", ");
			...
			System.out.println("와이리스트(dash) " + i + ":" + y_list);
